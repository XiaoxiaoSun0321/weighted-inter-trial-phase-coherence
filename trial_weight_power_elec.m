function trial_weights_elec = trial_weight_power_elec(eeg_data,srate,target_band,total_band,elec_used)
%function trial_weight_power_elec calculates the weights of each trial based
% on its power of interested frequency band. It calculates all electrodes 
% within the ROIs individually. 
...
%%%%%%%%%%%%%OUTPUT%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% trial_weights_elec: trial weights calculated based on power for each    %
%                    each electrode.the sum of trial_weights should be 1. %                 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
relative_power = nan(trial_num,length(elec_used));
for ix_trial = 1:trial_num 
    ...
    % Power of each electrode within the chosed ROIs
    for ix_chan = 1:length(elec_used)
        power_used = data_pw(:,elec_used(ix_chan));
        ...
        relative_power(ix_trial,ix_chan) = power_used_band/power_used_band_total;
    end
end

% Check if all trials are properly calculated
if sum(sum(isnan(relative_power)))>0 
    disp('Not all trials are calculated, please check for errors.')
    return;
end

trial_weights_elec = relative_power./sum(relative_power);
